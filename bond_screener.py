...
all_securities=all_boards_securities
...
def volumes(
	...
	# Находим суммы соответствующих объёмов для нужных интервалов времени
	res_table={}
	...
	for interval in needed_intervals:
		...
		volumes_interval=benchmark_dates[benchmark_dates.index<interval]
		for col_name in vol_sums:
			try:			
				vol_sums[col_name]=volumes_interval[col_name].sum()
				res_table.update({'{}_{}'.format(col_name,interval):vol_sums[col_name]})
			except KeyError:
				res_table.update({'{}_{}'.format(col_name,interval):0})

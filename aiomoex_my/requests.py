...
def _make_query(
    *, q=None, interval=None, start=None, end=None, table=None, columns=None, date=None):
...
def _get_table(data, table):
    ...
    #я переписал функцию, чтобы извлекала несколько таблиц
    ret_data={}
    try:
        if type(table)==list:
            if len(table)==1:
        ...
        raise client.ISSMoexError(f"Отсутствует таблица в данных")
    return ret_data
...
async def get_market_history(
    ...
    columns=(),#"BOARDID", "TRADEDATE", "CLOSE","LEGALCLOSEPRICE", "VOLUME", "VALUE", "NUMTRADES"
    market="shares",
    engine="stock",
):
...
async def get_board_history_all(
    date=None,
    board="TQCB",
    market="bonds",
    engine="stock",
    ...
    """
    Моя функция.
    Получить неценовые данные по всем бумагам в выбранном режиме торгов на выбранную дату
    Колонки выбрать нельзя - там сразу 3 таблицы, у них разные колонки. Под стандартную схему изначального sdk не подходит
    Выбор колонок реализован потом (не в этой функции), когда склеиваются уже 3 таблицы
    """
    url = (
        f"https://iss.moex.com/iss/engines/{engine}/markets/{market}/"
        f"boards/{board}/securities.json"
    )
    table = ["securities","marketdata","marketdata_yields"]
    # print('table ', table)
    # print('url ', url)

    query = _make_query(date=date, table=table, columns=None)

    # print('query ', query)

    return await _get_short_data(url, table, query)

async def bond_cash_flows(
    security=None,
    market="bonds",
    engine="stock",
):
    """
    Моя функция.
    Подгружает данные по амортизации облигаций и купону с учётом амортизации
    """
    url = (
        f"https://iss.moex.com/iss/statistics/engines/{engine}/markets/{market}/bondization/{security}.json"
    )
    table = ["amortizations","coupons"]
    # print('table ', table)
    # print('url ', url)

    query = _make_query(table=table, columns=None)
